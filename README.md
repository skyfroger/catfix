...
Чтобы указать код скрипта Yandex Метрики, используется переменная окружения `VITE_YM`.

Для размещения проекта в подкаталог необходимо задать значения переменной `VITE_BASE_URL`.
